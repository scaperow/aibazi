...
export class SolarTerm {
  ...
  static getYearTermDayList(year: number): number[] {
    const binData = TERM_SAME_HEX[TERM_LIST[Math.abs(year - FIRST_YEAR)]].toString(2)
    const res = []
  ...
  static getMonthTerms(year: number, month: number): [number, number] {
    ...
    const data = TERM_SAME_HEX[TERM_LIST[Math.abs(year - FIRST_YEAR)]].toString(2).padStart(48, '0')
